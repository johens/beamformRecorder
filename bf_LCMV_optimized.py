...
class LCMVOptimized:
    def __init__(self, n_channels, sample_rate, frame_len, hop, reg, diag_load, gain_db, limiter,
                 positions, target_az, target_el, null_azimuths, null_el, cov_alpha):
        ...
        self.null_azimuths = null_azimuths if isinstance(null_azimuths, list) else [null_azimuths]
        ...
        self.a_nulls = [steering_vector(self.freqs, self.positions, az, self.null_el) for az in self.null_azimuths]
    ...
    def _compute_weights(self):
        for f in range(self.n_freqs):
            ...
            # Null steering vectors
            a_nulls = []
            for a_n in self.a_nulls:
                a_null = a_n[f].reshape(-1, 1)
                a_null = a_null / (np.linalg.norm(a_null) + 1e-10)
                a_nulls.append(a_null)

            # Constraint matrix: [a_target | a_null_1 | a_null_2 | ...]
            C = np.hstack([a_t] + a_nulls)
            
            # Desired response: [1, 0, 0, ...]
            n_constraints = C.shape[1]
            d = np.zeros((n_constraints, 1), dtype=np.complex64)
            d[0, 0] = 1.0
            ...
            # Compute LCMV weights: w = Rf^-1 * C * (C^H * Rf^-1 * C)^-1 * d
            try:
...
def main():
    parser = argparse.ArgumentParser(description='Optimized real-time LCMV with covariance estimation and multiple nulls')
    parser.add_argument('--output', default='recorder_output/records/audio_lcmv_opt.wav', help='Output WAV file path')
    ...
    parser.add_argument('--diag-load', type=float, default=0.1, help='Diagonal loading factor (0-1)')
    parser.add_argument('--cov-alpha', type=float, default=0.95, help='Covariance smoothing (0-1, higher=slower update)')
    parser.add_argument('--gain', type=float, default=20.0, help='Output gain in dB')
    ...
    parser.add_argument('--target-azimuth', type=float, default=0.0, help='Target source azimuth (degrees)')
    parser.add_argument('--target-elevation', type=float, default=0.0, help='Target source elevation (degrees)')
    parser.add_argument('--null-azimuths', type=str, default='180', help='Null azimuth(s): single value or comma-separated (e.g. 180,150,210)')
    ...
    # Parse null azimuths
    null_azimuths = [float(x.strip()) for x in args.null_azimuths.split(',')]
    ...
    beamformer = LCMVOptimized(
        ...
        null_azimuths=null_azimuths,
        null_el=args.null_elevation,
        cov_alpha=args.cov_alpha,
    )

    debug('LCMV Optimized capture started', channels=n_channels, sample_rate=args.sample_rate,
          target=f'{args.target_azimuth}°/{args.target_elevation}°',
          nulls=f'{null_azimuths}°/{args.null_elevation}°')
    ...
    with wave.open(args.output, 'wb') as wf:
        ...
        try:
            ...
            debug('Stopping LCMV Optimized capture')
        finally:
            stream.stop_stream()
            stream.close()
            pa.terminate()
