"""
Adaptive-width Frost 3D tracker (recognize but do not steer into reject range).

How to run:

    python bf_Frost_adaptive_3d_null_track_adaptive_width_recognize_nosteer.py

Stable profile:

    python bf_Frost_adaptive_3d_null_track_adaptive_width_recognize_nosteer.py --track-duration 1.0 --lock-confidence-thresh 0.20 --release-confidence-thresh 0.12 --doa-bin-stride 6 --ui-fps 3

On RPi 5:
    python3 bf_Frost_adaptive_3d_null_track_adaptive_width_recognize_nosteer.py --device-index 1 --track-duration 1.0 --lock-confidence-thresh 0.20 --release-confidence-thresh 0.12 --doa-bin-stride 6 --ui-fps 3
...
def main():
    parser = argparse.ArgumentParser(description='Adaptive Frost 3D tracker with recognize-but-no-steer reject behavior')
    parser.add_argument('--output', default='recorder_output/records/audio_frost_adaptive_3d_null_track_adaptive_width_recognize_nosteer.wav', help='Output WAV file path')
    ...
    if ui_enabled and not display_env:
        debug('UI disabled: no DISPLAY/WAYLAND_DISPLAY found; running headless mode')
        ui_enabled = False
    ...
    debug(
        'Adaptive-width Frost tracker (recognize-no-steer) started',
    ...
    try:
        ...
        debug('Stopping adaptive-width Frost tracker (recognize-no-steer)')
    finally:
        wf.close()
        stream.stop_stream()
        stream.close()
